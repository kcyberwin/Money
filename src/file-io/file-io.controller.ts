...
export class FileIoController {
  ...
  async parseFile(@UploadedFile() file: Express.Multer.File) {
    ...
    try {
      let parseResults;
      this.logger.debug(
        `Parsing File ${file.originalname} mimetype ${file.mimetype}`,
        context,
      );
      switch (file.mimetype) {
        ...
        case 'application/octet-stream':
          if (file.originalname?.includes('.ofx')) {
            parseResults = this.fileIoService.processOfxFile(file);
          } else {
            parseResults = this.fileIoService.unsupportedFile(file);
          }
          break;
        default:
          parseResults = this.fileIoService.unsupportedFile(file);
